...
class UserProvider extends ChangeNotifier {
  ...
  Future<Response> loginUser(String email, String password, GlobalKey<ScaffoldState> errorScaffoldKey) async {
  ...
  Future<Response> registerUser(UserRegistration userRegistration, GlobalKey<ScaffoldState> errorScaffoldKey) async {
    Response<dynamic> response = await _authSource.register(userRegistration.email, userRegistration.password);
    if (response is Success<UserCredential>) {
      String id = response.value.user!.uid;
      response = await (_storageSource.uploadUserProfilePhoto(userRegistration.localProfilePhotoPath, id)
          as FutureOr<Success<UserCredential>>);

      if (response is Success<String>) {
        String profilePhotoUrl = response.value as String;
        AppUser user =
            AppUser(id: id, name: userRegistration.name, age: userRegistration.age, profilePhotoPath: profilePhotoUrl);
  ...
  void updateUserProfilePhoto(String localFilePath, GlobalKey<ScaffoldState> errorScaffoldKey) async {
    isLoading = true;
    notifyListeners();
    Response<dynamic> response = await _storageSource.uploadUserProfilePhoto(localFilePath, _user!.id);
    isLoading = false;
    if (response is Success<String>) {
